...
export const commands: Chat.ChatCommands = {
	hangman: {
		...
		addrandom(target, room, user) {
  		...
  		if (parts.length < 2) {
    		return this.errorReply("Usage: /hangman addrandom word, hint, hint, ...");
			}
			// validation
			...
			const hints = [];
				...
				if (hint.length > MAX_HINT_LENGTH) {
				return this.errorReply(`Hint "${hint}" is too long (max ${MAX_HINT_LENGTH} characters).`);
		}
	hints.push(hint);
	}
			
			if (!hangmanData[room.roomid][phrase]) hangmanData[room.roomid][phrase] = { hints: [] };
			hangmanData[room.roomid][validated.phrase].hints.push(...hints);
			Hangman.save();
			this.privateModAction(`${user.name} added a random hangman with ${Chat.count(hints.length, 'hints')}.`);
			this.modlog(`HANGMAN ADDRANDOM`, null, `${validated.phrase}: ${hints.join(', ')}`);
		},
		rr: 'removerandom',
